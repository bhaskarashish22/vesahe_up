...
$idm = $_GET['link'];
$pclink = $_GET['pclink'];
$presults = mysql_query("SELECT *FROM projectpages where link='$idm' and pclink='$pclink'");
?>
...
<head>
   ...
   <title><?php if($count == 0) { ?> Sorry No Project Found <?php } else { ?> <?=$rows['fname'];?> <?php } ?> </title>
...
<a href="<?=$urlroot;?>" class="btn btn-danger"> Click Here To Home Page </a>
   ...
   <?php } else { ?>
	...
	$tbl_name="projectpages";		//your table name
	// How many adjacent pages should be shown on each side?
	$adjacents = 3;
			$query = "SELECT COUNT(*) as num FROM $tbl_name where link='$idm' and pclink='$pclink' order by id desc";
	$total_pages = mysql_fetch_array(mysql_query($query));
	$total_pages = $total_pages[num];
	
	/* Setup vars for query. */
	$targetpage = "projects.php?pclink=".$_GET['pclink']."&link=".$_GET['link']; 	//your file name  (the name of this file)
	...
	/* Get data. */
	$sql = "SELECT *from $tbl_name where link='$idm' and pclink='$pclink' order by id desc LIMIT $start, $limit";
	...
	?>            
                    ...
                    <div class="ashade-col col-4">  
						<div class="ashade-gallery-item ashade-grid-item">
							<a href="<?=$urlroot;?>project-detail.php?pages=<?=$rowdata['pages'];?>">
								<img src="<?=$urlrooot;?>admin/uploads/projects/<?=$rowdata['photo'];?>" class="works-img" data-src="<?=$urlrooot;?>admin/uploads/projects/<?=$rowdata['photo'];?>" alt="<?=$rowdata['name'];?>" class="lazy">
